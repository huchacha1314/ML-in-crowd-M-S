...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	public void update(final double simTimeInSec) {
		...
		LinkedCellsGrid<Pedestrian> linkedCellsGrid = new LinkedCellsGrid<>(new VRectangle(topography.getBounds()), 1);
		DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();

		 for (Pedestrian p : c.getElements()){
			 linkedCellsGrid.addObject(p);
		 }
		 System.out.println(linkedCellsGrid.size());
		 if (linkedCellsGrid.size() > 0) {

			 for (Pedestrian p : linkedCellsGrid) {
				 ...
				 if (getGroup(p).getID() == SIRType.ID_SUSCEPTIBLE.ordinal()) {
				 	...
				 	for (Pedestrian p_neighbor : linkedCellsGrid.getObjects(p.getPosition(), attributesSIRG.getInfectionMaxDistance())) {
						 //for every neighbor in the radius of p
					 	if (getGroup(p_neighbor).getID() != SIRType.ID_INFECTED.ordinal()) {
							continue;// when p_neighbor is not infected in cannot infect p
					 	}

					 	//if (this.random.nextDouble() < attributesSIRG.getInfectionRate()) { // place where a person gets infected --> highly coupled to timestep rate
					 	if (this.random.nextDouble() < attributesSIRG.getInfectionRate() * 0.4/0.1) { // place where a person gets infected --> highly coupled to timestep rate
					 ...
					 //check if a pedestrian who is infected recovers
				 } else if (getGroup(p).getID() == SIRType.ID_INFECTED.ordinal()) {
					 if (this.random.nextDouble() < attributesSIRG.getRecoveryRate() * 0.4/0.1) {
						 SIRGroup g = getGroup(p);
						 elementRemoved(p);
